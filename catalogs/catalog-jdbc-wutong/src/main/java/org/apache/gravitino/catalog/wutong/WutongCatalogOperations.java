...
public class WutongCatalogOperations extends JdbcCatalogOperations {
  ...
  public void close() {
    super.close();
    try {
      // Unload the WuTongDB driver, only Unload the driver if it is loaded by
      // IsolatedClassLoader.
      Driver pgDriver = DriverManager.getDriver("jdbc:wutongdb://dummy_address:12345/");
      deregisterDriver(pgDriver);
    } catch (Exception e) {
      LOG.warn("Failed to deregister WuTongDB driver", e);
    }
  }
}
